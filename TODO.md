...
- [ ] 모바일 기본 폭에서 카드와 버튼 가독성 확인
- [ ] 태블릿/데스크톱에서 최대 폭 제한 유지
- [ ] 터치 영역이 충분한지 확인
- [ ] 텍스트 대비가 충분한지 확인
- [ ] 장식 요소가 학습 흐름을 방해하지 않는지 확인

## 9. 검증

- [x] `pnpm` 기준 린트 또는 타입체크 실행
- [ ] 수동 테스트 1: 단 미선택 시 시작 불가
- [ ] 수동 테스트 2: 단 선택 후 시작 가능
- [ ] 수동 테스트 3: 10문제 정상 출제
- [ ] 수동 테스트 4: 답 선택 시 즉시 채점
- [ ] 수동 테스트 5: 같은 문제에서 중복 채점 방지
- [ ] 수동 테스트 6: 마지막 문제 후 결과 화면 이동
- [ ] 수동 테스트 7: 오답 다시 풀기 정상 동작
- [ ] 수동 테스트 8: 오답 0개일 때 축하 문구와 버튼 노출 상태 확인
- [ ] 수동 테스트 9: 모바일 폭에서 버튼 터치성 확인

## 완료 기준

- [ ] 시작, 퀴즈, 결과 화면 전환이 끊김 없이 동작한다.
- [ ] 10문제 세션, 즉시 채점, 결과 계산이 정확하다.
- [ ] 오답 다시 풀기가 새 세션처럼 동작한다.
- [ ] 전체 톤이 밝고 예쁘고 깔끔한 문서 기준을 유지한다.
- [ ] 문서 기준 금지 사항을 침범하지 않는다.
